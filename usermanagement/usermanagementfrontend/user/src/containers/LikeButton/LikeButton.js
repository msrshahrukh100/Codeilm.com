    ...
    const avatars = this.state.likeUsers ?
      this.state.likeUsers.map(user => {
        return (<Tooltip
            ...
            >
              <Avatar key={user.id} onClick={() => this.props.history.push(`/u/${user.user.username}-${user.user.id}`)} alt={user.user.full_name} src={user.user.user_profile_pic} className={classes.avatar} />
            </Tooltip>)
      })
      : null;
