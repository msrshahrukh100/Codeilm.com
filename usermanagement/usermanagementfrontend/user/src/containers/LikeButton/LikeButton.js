...
class LikeButton extends React.Component {
  ...
  likeHandler = () => {
    ...
    axios.post('https://codeilm.com/api/v1/tutorials/like', postData)
    .then(response => {
      this.setState({
    ...
    const avatars = this.state.likeUsers ?
      this.state.likeUsers.map(user => {
        return (<Tooltip
            ...
            >
              <Avatar key={user.id} onClick={() => this.props.history.push(`/u/${user.user.id}/${user.user.username}`)} alt={user.user.full_name} src={user.user.user_profile_pic} className={classes.avatar} />
            </Tooltip>)
      })
      : null;
